...
def branches(x, block_num, global_layer, channels):
    out = []
    if global_layer:
        block_num=1
    for i in range(len(channels)):
        ...
        if global_layer:
            out.append(GlobalFeatureBlock_Diffusion(residual.shape[3], pde_args)(residual))
        else:
            out.append(residual)
    return out
...
def HighResolutionModule(x, channels, global_layer, multi_scale_output=True):
    residual = branches(x, 4, global_layer, channels)
    out = fuse_layers(residual, channels,
                      multi_scale_output=multi_scale_output)
    return out


def stage(x, num_modules, channels, global_layer, multi_scale_output=True):
    out = x
    for i in range(num_modules):
        if i == num_modules - 1 and multi_scale_output == False:
            out = HighResolutionModule(out, channels, global_layer, multi_scale_output=False)
        else:
            out = HighResolutionModule(out, channels, global_layer)

    return out


def hrnet_keras(input_size=(256, 256, 3), global_layer=False):
    channels_2 = [32, 64]
    ...
    st2 = stage(tr1, num_modules_2, channels_2, global_layer)
    tr2 = transition_layer(st2, channels_2, channels_3)
    st3 = stage(tr2, num_modules_3, channels_3, global_layer)
    tr3 = transition_layer(st3, channels_3, channels_4)
    st4 = stage(tr3, num_modules_4, channels_4, global_layer, multi_scale_output=False)
...
#     HRNet = hrnet_keras()
#     HRNet.summary()
#     input = tf.random.normal((32, 256,256,3))
#     print(HRNet.predict(input).shape)
