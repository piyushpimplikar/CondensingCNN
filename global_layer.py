...
import numpy as np
from tensorflow.keras.layers import AveragePooling2D, MaxPooling2D, ReLU, BatchNormalization, Lambda
...
def get_init_block( planes, block_type = 'default', args = None ):
    ...
    if block_type == 'BasicBlock':
        ...
        init_h = Conv2D(planes, kernel_size=(dw_kernel_size,dw_kernel_size), strides=(1,1), padding="same", groups=planes)
    elif block_type == 'FullConv':
        dw_kernel_size = args.get('dw_kernel_size', 3) 
        init_h = Conv2D(planes, kernel_size=(dw_kernel_size,dw_kernel_size), strides=(1,1), padding="same")
    elif block_type == 'PwConv':
        init_h = Conv2D(planes, kernel_size=(1,1), strides=(1,1))
...
class GlobalFeatureBlock_Diffusion(tf.Module):
    expansion: int = 1
    ...
    def __init__(
        ...
        no_diffusion = args.get('no_diffusion', True)
        no_advection = args.get('no_advection', True)
        if 'out_chs' in args:
            planes = out_chs
        ...
        self.no_advection = no_advection
        ...
        if self.nonlinear_pde:
            if args['custom_uv'] == '':
              if use_dw:
                ...
                #             act_layer=act_layer, noskip=True, pw_kernel_size=pw_kernel_size, se_layer=se_layer, norm_layer=norm_layer, drop_path_rate=drop_path_rate )
                 ...
                 if old_style:
                    self.convg  = Conv2D(planes, kernel_size=(dw_kernel_size,dw_kernel_size), strides=(1,1), padding="same", groups=planes)
                    self.convg1 = Conv2D(planes, kernel_size=(dw_kernel_size,dw_kernel_size), strides=(1,1), padding="same", groups=planes)
                 else:
                    # self.convg = Conv2D(planes, kernel_size=(dw_kernel_size, dw_kernel_size), strides=(1, 1),padding="same", groups=planes, dilation_rate=dilation, use_bias=False)
                    # self.convg1 = Conv2D(planes, kernel_size=(dw_kernel_size, dw_kernel_size), strides=(1, 1),padding="same", groups=planes, dilation_rate=dilation, use_bias=False)
            ...
            if constant_Dxy == False:
              if args['custom_dxy'] == '':
                if use_cDs == False:
                    if old_style:
                        self.convDx = Conv2D(planes, kernel_size=(dw_kernel_size,dw_kernel_size), strides=(1,1), padding="same", groups=planes)
                        self.convDy = Conv2D(planes, kernel_size=(dw_kernel_size,dw_kernel_size), strides=(1,1), padding="same", groups=planes)
                    else:
                        # self.convDx = Conv2D(planes, kernel_size=(dw_kernel_size, dw_kernel_size), strides=(1, 1),padding="same", groups=planes, dilation_rate=dilation, use_bias=False)
                        # self.convDy = Conv2D(planes, kernel_size=(dw_kernel_size, dw_kernel_size), strides=(1, 1),padding="same", groups=planes, dilation_rate=dilation, use_bias=False)
              ...
              if use_diff_eps:
                    self.bnDx = norm_layer()
                    self.bnDy = norm_layer()
              else:    
                    self.bnDx = norm_layer()
                    self.bnDy = norm_layer()
    ...
    def __call__(self, s0, s1 = None, drop_path=None):
        ...
        ux = (1. / (2*dx)) * ( tf.roll(g, dx, axis=1)  - tf.roll(g, -dx, axis=1) )
        vy = (1. / (2*dy)) * ( tf.roll(g1, dy, axis=2) - tf.roll(g1, -dy, axis=2) )
        ...
        for k in range(self.K):
            ...
            h = D  *   (   (1 - 2*Bx - 2*By) * h0 - 2 * E * h 
                         + (-Ax  + 2*Bx) * tf.roll(h, dx, axis=1)
                         + ( Ax  + 2*Bx) * tf.roll(h, -dx, axis=1)
                         + (-Ay  + 2*By) * tf.roll(h, dy, axis=2)
                         + ( Ay  + 2*By) * tf.roll(h, -dy, axis=2)
                        ) # + 2 * dt * f )
            if self.no_f == False:
                h = h + D * 2 * dt * f 
